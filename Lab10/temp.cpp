...
bool createFileOrDir(string filename, int FileOrDir)
{
	...
	{
		cout<<"directory blocks full\n";
		return false;
	}
	else if(FileOrDir + num_free_blocks < 2)
	{
		cout<<"not enough free blocks available\n";
		return false;
	}
	else
	{
...
void seekRead(string filename, int pos)
{
	if(index_blocks.find(filename) == index_blocks.end())
		...
		cout<<"File length exceeded\n";
	else
	{
		file_read_pointer[filename] = pos;
		cout<<"read pointer set\n";
	}
}
...
void fileRead(string filename, int length)
{
	if(index_blocks.find(filename) == index_blocks.end())
		...
		cout<<"File Read Pointer not initialised yet\n";
	else
	{
		...
		if(start > end)
			cout<<"Read pointer at end of file, read not possilbe\n";
		else
		{
			cout<<"Bytes read "<<start<<" - "<<end<<endl;
...
void fileWrite(string filename, string text)
{
	if(index_blocks.find(filename) == index_blocks.end())
		...
		cout<<"File Write Pointer not initialised yet\n";
	else
	{
		...
		if(extendFileSize(filename, space_reqd))
		{
			...
			cout<<"write could not be performed\n";
	}
}
...
void deleteDirectory(string dir)
{
	if(index_blocks.find(dir) != index_blocks.end())
		cout<<"Given name is that of a file\n";
	else
	{
		...
		else
		{
			...
			if(count == -1)
				...
				cout<<"directory has files, cannot be deleted\n";
			else
			{
				file_size.erase(dir);
