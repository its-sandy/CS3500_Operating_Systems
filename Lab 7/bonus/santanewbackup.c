...
int n, prob;
int elves, reindeer;
sem_t santaSem, reindeerSem, elfTex, mutex, elfHelpTex;
...
void *runner_santa(void *param)
{
	while(true)
	{
		...
		sem_wait(&mutex);
			...
			if(reindeer >= 9)
			{
				printf("SANTA starts to prepare sleigh\n");
				fflush(stdout);
				nanosleep((const struct timespec[]){{0, 200*(1000000L)}}, NULL);
				printf("SANTA finishes preparing sleigh\n");
				fflush(stdout);
...
void *runner_reindeer(void *param)
{
	...
	printf("REINDEER with ID = %d started hitching\n", reindeer_id);
	fflush(stdout);
	nanosleep((const struct timespec[]){{0, 150*(1000000L)}}, NULL);
	printf("REINDEER with ID = %d finished hitching\n", reindeer_id);
	fflush(stdout);
